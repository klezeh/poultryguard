// screens/add_temperature_humidity_screen.dart
...
class AddTemperatureHumidityScreen extends ConsumerStatefulWidget {
  final TemperatureHumidityRecord? existing; // Optional: for editing an existing record
  ...
  const AddTemperatureHumidityScreen({
    super.key,
    this.existing,
    this.recordKey,
  ...
  @override
  ConsumerState<AddTemperatureHumidityScreen> createState() => _AddTemperatureHumidityScreenState();
}

class _AddTemperatureHumidityScreenState extends ConsumerState<AddTemperatureHumidityScreen> {
  ...
  void initState() {
    ...
    if (widget.existing != null) {
      ...
      _temperatureController.text = widget.existing!.temperatureC.toString();
      _humidityController.text = widget.existing!.humidityPercent.toString();
  ...
  /// Saves or updates a temperature and humidity record in Hive and flags it for sync.
  void _saveTemperatureHumidityRecord() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_selectedBatchName == null || _selectedBatchName!.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please select a batch for this record.')),
          );
        }
        return;
      }
      ...
      try {
        final box = Hive.box<TemperatureHumidityRecord>('temperature_humidity_records'); // Use specific box
        ...
        if (widget.recordKey != null && widget.existing != null) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Temperature & Humidity record updated successfully!')),
            );
          }
        } else {
          // Add new record
          final newRecord = TemperatureHumidityRecord(
            date: _selectedDate,
            temperatureC: double.parse(_temperatureController.text),
            humidityPercent: double.parse(_humidityController.text),
            batchName: _selectedBatchName!,
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Temperature & Humidity record added successfully!')),
            );
          }
        }
        ...
        debugPrint('Error saving temperature and humidity record: $e');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to save temperature and humidity record: $e')),
          );
        }
      } finally {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.existing == null ? 'Add Temp & Humidity Record' : 'Edit Temp & Humidity Record',
          ...
          child: Column(
            ...
            children: [
              ...
              TextFormField(
                ...
                enabled: !_isSaving,
                onSaved: (value) => _temperatureController.text = value ?? '0.0',
              ),
              const SizedBox(height: 15),
              ...
              TextFormField(
                ...
                enabled: !_isSaving,
                onSaved: (value) => _humidityController.text = value ?? '0.0',
              ),
              const SizedBox(height: 15),
      ...
      bottomNavigationBar: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 10),
            ElevatedButton.icon(
              onPressed: _isSaving ? null : _saveTemperatureHumidityRecord,
              style: ElevatedButton.styleFrom(
                backgroundColor: primaryColor,
                foregroundColor: Colors.white,
                padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
