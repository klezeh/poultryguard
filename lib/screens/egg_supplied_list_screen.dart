...
import 'add_egg_supplied_screen.dart';

// CHANGE: Converted to ConsumerStatefulWidget
class EggSuppliedListScreen extends ConsumerStatefulWidget {
  const EggSuppliedListScreen({super.key});

  @override
  ConsumerState<EggSuppliedListScreen> createState() => _EggSuppliedListScreenState();
}

class _EggSuppliedListScreenState extends ConsumerState<EggSuppliedListScreen> {
  DateTime? _startDate;
  DateTime? _endDate;
  ...
  void _loadFilterDates() {
    final saved = _filterBox.get('egg_supplied_date_filter') as Map?;
  ...
  void _saveFilterDates() {
    _filterBox.put('egg_supplied_date_filter', {
  ...
  Future<void> _exportToCsv() async {
    ...
    try {
      final path = await CsvExporter.exportEggSuppliedToCsv(data);
  ...
  Widget build(BuildContext context) {
    final eggBox = Hive.box<EggSupplied>('egg_supplied');

    return Scaffold(
      appBar: AppBar(
        title: const Text('🥚 Eggs Supplied', style: TextStyle(color: Colors.white)),
        ...
        actions: [
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(context, MaterialPageRoute(builder: (_) => const AddEggSuppliedScreen()));
            },
          ),
        ],
      ),
      body: ValueListenableBuilder(
        ...
        builder: (context, Box<EggSupplied> box, _) {
          ...
          if (entries.isEmpty) {
            return const Center(child: Text('No egg supplied records for the selected date range.'));
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return Card(
                ...
                child: ListTile(
                  title: Text("${egg.quantity} eggs to ${egg.customerName}", style: const TextStyle(fontWeight: FontWeight.bold)),
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(builder: (_) => AddEggSuppliedScreen(existing: egg, eggKey: egg.key)),
                          );
                        },
                      ),
                      // --- THIS IS THE FIX ---
                        ...
                        onPressed: () async {
                           final confirm = await showDialog<bool>(
                            ...
                            builder: (context) => AlertDialog(
                              ...
                              content: Text('Are you sure you want to delete this supply record to ${egg.customerName}?'),
                              actions: [
