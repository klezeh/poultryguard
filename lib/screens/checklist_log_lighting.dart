// screens/add_lighting_screen.dart
...
import '../models/lighting_record.dart'; // Import the new LightingRecord model
...
class AddLightingScreen extends ConsumerStatefulWidget {
  final LightingRecord? existing; // Optional: for editing an existing record
  ...
  @override
  ConsumerState<AddLightingScreen> createState() => _AddLightingScreenState();
}

class _AddLightingScreenState extends ConsumerState<AddLightingScreen> {
  final _formKey = GlobalKey<FormState>();
  final TextEditingController _lightsOnController = TextEditingController();
  ...
  void initState() {
    ...
    if (widget.existing != null) {
      ...
      _lightsOnController.text = widget.existing!.lightsOnHours.toString();
      _lightsOffController.text = widget.existing!.lightsOffHours.toString();
  ...
  /// Saves or updates a lighting record in Hive and flags it for sync.
  void _saveLightingRecord() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_selectedBatchName == null || _selectedBatchName!.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Please select a batch for this lighting record.')),
          );
        }
        return;
      }
      ...
      try {
        final box = Hive.box<LightingRecord>('lighting_records'); // Use 'lighting_records' box
        ...
        if (widget.recordKey != null && widget.existing != null) {
          ...
          final updatedRecord = widget.existing!.copyWith(
            date: _selectedDate,
            lightsOnHours: double.parse(_lightsOnController.text),
            lightsOffHours: double.parse(_lightsOffController.text),
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Lighting record updated successfully!')),
            );
          }
        } else {
          // Add new record
          final newRecord = LightingRecord(
            date: _selectedDate,
            lightsOnHours: double.parse(_lightsOnController.text),
            lightsOffHours: double.parse(_lightsOffController.text),
            batchName: _selectedBatchName!,
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Lighting record added successfully!')),
            );
          }
        }
        ...
        debugPrint('Error saving lighting record: $e');
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Failed to save lighting record: $e')),
          );
        }
      } finally {
        if (mounted) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.existing == null ? 'Add Lighting Record' : 'Edit Lighting Record',
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              // Lights On Hours Input Field
              TextFormField(
                controller: _lightsOnController,
                decoration: InputDecoration(
                  labelText: 'Lights On (hours)',
                  labelStyle: TextStyle(color: primaryColor),
                  prefixIcon: Icon(Icons.lightbulb, color: primaryColor),
                  border: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(12),
                    borderSide: BorderSide(color: Colors.grey.shade300, width: 1.5),
                  ),
                ...
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Please enter lights on hours';
                  }
                  if (double.tryParse(value) == null || double.parse(value)! < 0 || double.parse(value)! > 24) {
                    return 'Enter a valid number between 0-24';
                  }
                  return null;
                },
                enabled: !_isSaving,
                onSaved: (value) => _lightsOnController.text = value ?? '0.0',
              ),
              const SizedBox(height: 15),

              // Lights Off Hours Input Field
              TextFormField(
                controller: _lightsOffController,
                decoration: InputDecoration(
                  labelText: 'Lights Off (hours)',
                  labelStyle: TextStyle(color: primaryColor),
                  prefixIcon: Icon(Icons.lightbulb_outline, color: primaryColor),
                  border: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(12),
                    borderSide: BorderSide(color: Colors.grey.shade300, width: 1.5),
                  ),
                ...
                validator: (value) {
                  if (value == null || value.isEmpty) {
                    return 'Please enter lights off hours';
                  }
                  if (double.tryParse(value) == null || double.parse(value)! < 0 || double.parse(value)! > 24) {
                    return 'Enter a valid number between 0-24';
                  }
                ...
                enabled: !_isSaving,
                onSaved: (value) => _lightsOffController.text = value ?? '0.0',
              ),
              const SizedBox(height: 15),
      ...
      bottomNavigationBar: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            const SizedBox(width: 10),
            ElevatedButton.icon(
              onPressed: _isSaving ? null : _saveLightingRecord,
              style: ElevatedButton.styleFrom(
                backgroundColor: primaryColor,
                foregroundColor: Colors.white,
                padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 12),
